...
Shader "Custom/Piku"
{
	Properties
	{
		...
		[ASEBegin]_Piku_Albedo("Piku_Albedo", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float2 uv_Piku_Albedo = packedInput.ase_texcoord1.xy * _Piku_Albedo_ST.xy + _Piku_Albedo_ST.zw;
				float4 tex2DNode20 = tex2D( _Piku_Albedo, uv_Piku_Albedo );
				
				surfaceDescription.Color = tex2DNode20.rgb;
				surfaceDescription.Emission = ( tex2DNode20 * _Emissive ).rgb;
		...
		Pass
		{
					...
					)
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 uv_Piku_Albedo = packedInput.ase_texcoord.xy * _Piku_Albedo_ST.xy + _Piku_Albedo_ST.zw;
				float4 tex2DNode20 = tex2D( _Piku_Albedo, uv_Piku_Albedo );
		...
		Pass
		{
					...
					)
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 uv_Piku_Albedo = packedInput.ase_texcoord3.xy * _Piku_Albedo_ST.xy + _Piku_Albedo_ST.zw;
				float4 tex2DNode20 = tex2D( _Piku_Albedo, uv_Piku_Albedo );
...
ASEEND*/
//CHKSM=DED73825A67B54EB0B86AFDAE445374DBD571167